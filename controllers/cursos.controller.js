      ...
      db.query(
        ...
        (err, rows) => {
          ...
          if (rows.affectedRows == 1)
            res.status(200).json({ respuesta: "Curso actualizado con exito" });
        }
      );
    } catch (err) {
      res.status(500).send(err.message);
    ...
    try {
      db.query(
        ...
        (err, rows) => {
          ...
          if (rows.affectedRows == 1)
            res.status(200).json({ respuesta: "Curso eliminado con exito" });
        }
      );
    } catch (err) {
      res.status(500).send(err.message);
      ...
      db.query(
        ...
        (err, rows) => {
          err
            ...
            : res
                .status(201)
                .json({ respuesta: "Estudiante registrado con exito" });
        }
      );
    } catch (err) {
      res.status(500).send(err.message);
